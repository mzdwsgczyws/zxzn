/**
 * 国家 + 中国省级 / 地级（二级市）选项，供档案「手写地区」三档选择。
 * 直辖市、省、自治区均含主要地级市；数据可随需扩充。
 */
const COUNTRIES = [
  { code: 'CN', name: '中国' },
  { code: 'OTHER', name: '其他' }
...
/** [省或直辖市名, 该市下可选「二级城市」列表（含省会及主要地级市）] */
const PROVINCES_AND_CITIES = [
  ...
  [
    '新疆维吾尔自治区',
    ['乌鲁木齐市', '克拉玛依市', '吐鲁番市', '哈密市', '昌吉回族自治州', '博尔塔拉蒙古自治州', '巴音郭楞蒙古自治州', '阿克苏地区', '喀什地区', '伊犁哈萨克自治州']
  ],
...
function getProvinceLabels() {
  return PROVINCES_AND_CITIES.map((row) => row[0])
}

function getCityLabels(provinceIndex) {
  const row = PROVINCES_AND_CITIES[provinceIndex]
