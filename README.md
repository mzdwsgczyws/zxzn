...
面向「自我观察与文化参考」的工具型小程序：**今日灵签**（含签图、建议与可选混元延展）、**道性十六型**情景测验、**量化修行记录**与轻量分析、**个人档案**（签运增强）；首页提供 **灵签展馆**（每卦首次结缘时间）与 **成就展馆**（签缘成就与长按趣评）。数据默认**仅存本机**（`wx.setStorage`），未接自有后端业务库。
...
| 动线 | 说明 |
|------|------|
| **每日一问** | 首页启签 → 摇签 → 揭签 → 读诗/释义/建议；可选 AI 漫谈（需云开发）。 |
| **自察分型** | 道性测验 → 十六型结果与修行提示；可重做，以最后一次为准。 |
| **量化记录** | 修行页填睡眠、情绪等滑动项 → 保存 → 刷新分析得侧重点（非诊断）。 |
| **档案与签运** | 个人档案补齐签运增强项后，抽签少打扰、建议与释义更贴上下文。 |
| **展馆** | 灵签展馆按时间看「首获」图鉴；成就展馆用趣味条件解锁勋章，长按看评语。 |
...
- **等第**由 `utils/lots.js` 中 **`TIERS_BY_LOT_ID[lotId]`** 逐项绑定，使签诗、释义的整体语气与等第一致（避免「讼、否、剥」等偏警示文案却标上上/上之类错位）。
- **分布仍固定**（与建议池、签图配色统计一致）：上上 8、上 16、中 24、下 10、下下 6；加载时会校验数组长度与各档计数。
- `tierForId(id)` 读上述映射；`lotId` 仍由当日哈希决定，仅决定抽到哪一签，不改变各签绑定的等第。
...
- **灵签展馆**（`pages/lot-hall`）：六十四卦按「**首次抽到**」的时间升序列出，展示签名、等第、首次日期时刻。
...
除年龄、性别、出生月、**参考地区**（国家 → 省/直辖市 → 二级城市 + 可选详细文字；保存时 Geocoding 写入 `latitude`/`longitude`/`locationName`）外，可选填 **签运增强** 字段（见第三节），用于：
...
- **功能漏斗**：AI 漫谈点击次数、揭签率（需与当日签缓存关联）、测验中途退出题号。  
...
- 签文与等第映射调整时，同步检查 **`lots-data.js`**、**`lots.js`（`TIERS_BY_LOT_ID` 分布 8/16/24/10/6）**、**`fortune.js` / `lottery-advice.js`** 的注释与本文档第三节。
...
- **导出**：将修行记录或某次签文导出为文本（`wx.share` / 复制），便于用户自行备份。
